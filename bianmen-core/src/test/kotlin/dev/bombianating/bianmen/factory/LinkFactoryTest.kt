...
import dev.bombinating.bianmen.factory.LinkFactory.link
...
class LinkFactoryTest : AbstractWicketTest() {
    ...
    fun `renderBodyOnly Test`() = listOf(true, false, null)
        ...
        .map {
            dynamicTest("use model=${it.second != null}, renderBodyOnly=${it.first}") {
                if (it.second == null) {
                    link(id = COMP_ID, renderBodyOnly = it.first)
                } else {
                    link(id = COMP_ID, model = it.second, renderBodyOnly = it.first)
                }.test {
    ...
    fun `escapeModelStrings Test`() = listOf(true, false, null)
        ...
        .map {
            dynamicTest("use model=${it.second != null}, escapeModelStrings=${it.first}") {
                if (it.second == null) {
                    link(id = COMP_ID, escapeModelStrings = it.first)
                } else {
                    link(id = COMP_ID, model = it.second, escapeModelStrings = it.first)
                }.test {
    ...
    fun `visibleWhen Test`() = listOf(Model.of(true), Model.of(false), null)
        ...
        .map {
            ...
            if (it.second == null) {
                link(id = COMP_ID, visibleWhen = it.first)
            } else {
                link(id = COMP_ID, model = it.second, visibleWhen = it.first)
            }.test {
    ...
    fun `enabledWhen Test`() = listOf(Model.of(true), Model.of(false), null)
        ...
        .map {
            dynamicTest("use model=${it.second != null}, enabledWhen=${it.first?.obj}") {
                if (it.second == null) {
                    link(id = COMP_ID, enabledWhen = it.first)
                } else {
                    link(id = COMP_ID, model = it.second, enabledWhen = it.first)
                }.test {
    ...
    fun `refType Test`() = ComponentReferenceType.values()
        ...
        .map {
            ...
            if (it.second == null) {
                link(id = COMP_ID, refType = it.first)
            } else {
                link(id = COMP_ID, model = it.second, refType = it.first)
            }.test {
    ...
    fun `add Behavior Test`() = listOf("test".model(), null).map {
        dynamicTest("model=${if (it == null) "<not set>" else "<set>"}") {
            ...
            if (it != null) {
                link(id = COMP_ID, model = it) { add(behavior) }
            } else {
                link(id = COMP_ID) { add(behavior) }
            }.test {
    ...
    fun `onClick Test`() = listOf("test".model(), null).map {
        dynamicTest("model=${if (it == null) "<not set>" else "<set>"}") {
            var i: Int = 0
            if (it != null) {
                link(id = COMP_ID, model = it) { onClick { i++ } }
            } else {
                link(id = COMP_ID) { onClick { i++ } }
            }.test {
