...
import dev.bombinating.bianmen.factory.ButtonFactory.button
...
class ButtonFactoryTest : AbstractWicketTest() {
    ...
    fun `renderBodyOnly Test`() = listOf(true, false, null).map {
        dynamicTest("renderBodyOnly=$it") {
            Form<String>(FORM_ID).add(button(id = COMP_ID, renderBodyOnly = it)).test(markup = markup()) {
    ...
    fun `escapeModelStrings Test`() = listOf(true, false, null).map {
        dynamicTest("escapeModelStrings=$it") {
            Form<String>(FORM_ID).add(button(id = COMP_ID, escapeModelStrings = it)).test(markup = markup()) {
                val c = tester.getComponentFromLastRenderedPage("$FORM_ID:$COMP_ID")
    ...
    fun `visibleWhen Test`() = listOf(Model.of(true), Model.of(false), null).map {
        dynamicTest("visibleWhen=${it?.obj}") {
            Form<String>(FORM_ID).add(button(id = COMP_ID, visibleWhen = it)).test(markup = markup()) {
    ...
    fun `enabledWhen Test`() = listOf(Model.of(true), Model.of(false), null).map {
        dynamicTest("enabledWhen=${it?.obj}") {
            Form<String>(FORM_ID).add(button(id = COMP_ID, enabledWhen = it)).test(markup = markup()) {
    ...
    fun `refType Test`() = ComponentReferenceType.values().map {
        dynamicTest("refType=$it") {
            Form<String>(FORM_ID).add(button(id = COMP_ID, refType = it)).test(markup = markup()) {
                tester.getComponentFromLastRenderedPage("$FORM_ID:$COMP_ID").check(it)
            }
        }
    }
    ...
    @Test
    fun `behaviors Test`() {
        Form<String>(FORM_ID).add(button(id = COMP_ID) { add(AttributeAppender("class", "foo")) })
            .test(markup = markup()) {
